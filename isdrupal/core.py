"""isdrupal.core — the Drupal detection engine.

Pure detection logic shared by every front-end. It knows nothing about argparse
or Flask: callers pass a `DetectConfig` and get back a `DrupalResult`.

Public surface: `make_session`, `detect_drupal`, `format_result`, plus the
`Signal`/`DrupalResult` dataclasses and `normalize_url`.
"""
...
def set_debug(enabled: bool) -> None:
    """Toggle stderr debug tracing (the CLI's -d/--debug)."""
    global _debug
    _debug = enabled
...
def browser_cf_bypass(url: str, session: requests.Session) -> bool:
    ...
    CLI-only: the web front-end never sets browser_fallback, since this blocks
    on stdin and needs a display.
    """
    try:
...
def detect_drupal(raw_url: str, session: requests.Session, cfg: DetectConfig) -> DrupalResult:
    ...
    # If blocked and browser fallback requested, open browser, let user clear the
    # challenge, then re-fetch the homepage with the resulting cookies
    ...
    if not cfg.fast:
        ...
        dprint("Skipping probes (fast mode)")
